...
class Snoozed
{
    ...
    {
        $snoozedMonitors = MonitorRepository::getSnoozed();

        if (! $snoozedMonitors->count()) {
            return;
        }

        ConsoleOutput::warn('Snoozed monitors');
        ConsoleOutput::warn('===================');

        $rows = $snoozedMonitors->map(function (Monitor $monitor) {
            $certificateFound = '';
